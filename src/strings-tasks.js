...
function sumOfCodes(str) {
  if (str !== undefined && str !== null) {
    return str.split('').reduce((ac, symbol) => ac + symbol.charCodeAt(0), 0);
  }
  return 0;
}
/**
...
function isPalindrome(str) {
  const firstStr = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
...
function findLongestWord(sentence) {
  ...
  const arrayWithCountChar = arrayWords.map((word) => word.length);
  const maxCountChar = Math.max(...arrayWithCountChar);
  return arrayWords[arrayWithCountChar.indexOf(maxCountChar)];
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (charCode >= 65 && charCode <= 90) {
      startStr += String.fromCharCode(((charCode - 65 + 13) % 26) + 65);
    } else if (charCode >= 97 && charCode <= 122) {
      startStr += String.fromCharCode(((charCode - 97 + 13) % 26) + 97);
    } else {
      startStr += str[i];
    }
  }
